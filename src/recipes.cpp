...
bool readOneLine(char* line){
  ...
  int idx = 0;
  int line_len = 0;
  char * line_read = new char[MAX_LENGTH];
  ...
  while (myIO->recipeFileavailable() && not linefound){
    ...
    while (myIO->recipeFileavailable()) { 
      //myIO->recipeFileread(line_read);                  // read one single character into ch
      line_len = strlen(line_read);
      if (strlen(line_read) != 0 || idx + line_len < MAX_LENGTH - 1)
      {                                  // if it is not \n, than add it to line
	        strcpy(line, line_read);
	        idx += line_len;
			myIO->serialPrintln(idx);
      }
      else
      {
        endofline = true;
...
int readColumns(char* line, const char* ch, char * col[], int numCol){
  ...
  while (strlen(line) > 1 && i<numCol){
    ...
    myIO->serialPrint(i);myIO->serialPrint((char*)" , delimeter at :");
    myIO->serialPrint(pos);myIO->serialPrint((char*)" , substring is :");
...
bool stripQuotes(char * str){
  ...
  myIO->serialPrint((char *)"stripQuotes: ");
  myIO->serialPrint((char *)str);
  strncpy(temp, str + 1, strlen(str)-2);
  str = temp;
  myIO->serialPrint((char *)" -> ");myIO->serialPrint((char *)str);
...
{
  myIO->serialPrint((char*)"comparestring: ");myIO->serialPrint(s1);myIO->serialPrint(s2);
...
int getversion(char* str){
  int version = 0;
  myIO->serialPrint((char*)"getVersion: str= ");myIO->serialPrint(str);
  if (comparestring(str,"MAGOD1")) {version = 1;}
  else
    if (comparestring(str,"MAGOD2")) {version = 2;}
    else {myIO->serialPrint((char *)"Version not recognized");} 
  return version;
}
...
bool readSequence(sequence& seq){
  ...
  while(not endSequence && readOneLine(line)){ 
    myIO->serialPrint((char*)"In readSequence line : ");myIO->serialPrint((char*)line);
    if (line[0] == '@'){
      numCol = readColumns(line, ",", col, N);
      ...
      myIO->serialPrint((char*)"sequenceNumber: ");myIO->serialPrintln(sequenceNumber);
    }
  }
...
int recipes::LoadRecipes(recipe recipes_array[]){
  ...
  while(myIO->recipeFileavailable() && not error){
    ...
    if (readOneLine(line)) {
      ...
      myIO->serialPrint((char*)"Line ");myIO->serialPrint(linecount);
      myIO->serialPrint(" : ");myIO->serialPrintln(line);
      if(line[0] == '@'){
	...
	// Parameter list
		if (comparestring(col[1],"Parameters")) {
			//myIO->serialPrintln((char*)"Parameters");
	...
	// Recipe
	  ...
	  //Serial.print("Recipe: ");Serial.println(col[2]);
			...
			myIO->serialPrintln(recipeNumber);
  ...
  //myIO->serialPrint(recipeNumber);myIO->serialPrintln((char*)" recipes found: ");
  for (int i=0;i<recipeNumber;i++){
    myIO->serialPrint(i);myIO->serialPrint((char*)" : \"");myIO->serialPrint(recipes_array[i].name);
    myIO->serialPrint((char*)"\" Number of steps : ");
    myIO->serialPrint(recipes_array[i].recipe_sequence.length+1);
